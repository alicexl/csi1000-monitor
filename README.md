...
反例对照（为什么不能只看 d_far）：
- 正常 backwardation：d_near=+5%, d_far=+10% → roll_yield=+5%（展期赚钱）✅
- 曲线扁平：d_near=+8%, d_far=+5% → roll_yield=−3%（展期亏钱）❌ 旧逻辑会漏检
...
### 4. 贴水判断基于曲线斜率（roll_yield，不是单合约贴水）
...
历史上曾用 `next_month_discount > 0` 作为信号条件。但反例：d_near=+8%, d_far=+5% 时，d_far>0（满足旧条件）但 roll_yield=−3%（展期亏钱）→ 旧逻辑漏检。改为 roll_yield 后能正确识别曲线异常。
