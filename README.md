...
> 买入 IM 期货多头，持有到交割，期货会收敛到现货，**这部分差价就是白赚的贴水收益**。

但不是无脑多头。两个风险：

1. **估值过高时买入** → 指数本身下跌把贴水收益亏光
2. **期货转升水时买入** → 贴水消失，策略前提失效

所以策略是：

> **估值低（PE 历史分位 <50%）且有贴水（期货 < 现货）时买；估值过高（>85%）或贴水消失时卖。**
...
[2] _generate_report() — 读 DB → 评估信号 → 写 signals 表 + 渲染报告
    ...
    │   └─ → {pe_ttm, pe_ttm_pct, contracts, otm_call, ...}
    │
    ├─ _extract_signal_metrics()
    │   抽出 signals 需要的 3 个数：pe_ttm_pct_10y / current_month_discount / current_month_days
    │   （当月临近交割时 fallback 到下月）
    │
...
策略根据 `position.status` 走**空仓侧**或**持仓侧**分支：

| 持仓 | 信号 | priority | 触发条件 |
|---|---|---|---|
| 空仓 | `entry` | 2 | PE<50% 且 贴水>0 |
| 空仓 | `warn_entry` | 4 | PE 50-60%，**或** PE 够低但升水 |
| 空仓 | `wait` | 5 | 兜底（其他情况）|
| 持仓 | `reduce` | 1 | PE>85% **或** 贴水≤0（双触发都展示）|
...
### 4. 当月 fallback（`_extract_signal_metrics`）

当当月合约剩余天数 <7 天时，信号评估的贴水指标 fallback 到下月合约。和上面的交割日标签滚动互补：
- **标签滚动** 处理 days=0（已交割）
- **fallback** 处理 days=1-6（临交割但未交割）
...
- `test_reporter.py` — Markdown 渲染 + 操作建议
- `test_monitor.py` — CLI 集成
- `test_e2e.py` — 端到端对比报告
