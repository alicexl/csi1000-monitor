...
- **展期收益** = 近月与远月的价差斜率 = 下月年化贴水 − 当月年化贴水。下月比当月更深贴水（曲线向下倾斜）→ 展期能吃到差价
- 吃贴水策略看的是**曲线斜率（roll_yield）**，**不是单合约绝对贴水**——绝对贴水存在 ≠ 展期能赚钱（反例：当月 +8%/下月 +5%，两合约都贴水但 roll_yield = −3%，展期反向亏）。当月基差临近交割还会归零，是噪音不是失效信号
...
吃贴水策略本质是**展期收益**：每展期一次（卖近月、买远月）吃到一份价差。这个价差 = P_近 − P_远 = `下月年化贴水 − 当月年化贴水`（即曲线斜率），**不是**任何单合约对现货的绝对贴水。

三种曲线形态：
- **正常 backwardation**（曲线向下倾斜）：d_near=+5%, d_far=+10% → roll_yield=+5%（展期赚钱）✅
- **曲线扁平**：d_near=+8%, d_far=+5% → roll_yield=−3%（展期亏钱）❌
- **曲线倒挂**：d_near=+8%, d_far=+3% → roll_yield=−5%（展期亏更多）❌

绝对贴水存在 ≠ 展期能赚钱。只有曲线斜率为正时，反复展期才能持续吃到价差。
...
### 3. 贴水判断基于曲线斜率（roll_yield，不是单合约贴水）

吃贴水策略本质是**展期收益**：每展期一次（卖近月、买远月）吃到一份价差。

```
roll_yield = 下月年化贴水 − 当月年化贴水
           > 0 → 曲线向下倾斜（正常 backwardation），展期能吃到价差
           ≤ 0 → 曲线扁平或倒挂，展期会反向亏钱
```

- **判断指标用曲线斜率**（roll_yield，决定展期能吃多少）
- **不用单合约对现货的绝对贴水**（绝对贴水存在 ≠ 展期能赚钱；曲线倒挂时即使两合约都有贴水，展期也亏）
- 当月/下月绝对贴水仍展示在期货合约表里作为参考
...
报告中的「预期收益」panel 基于**杨康平《股指期货吃贴水策略》PDF** 的核心框架（简化版）：
...
**展期收益不计入此 panel**：
- 吃贴水策略的核心收益是展期收益（roll_yield = 曲线斜率）
- 但曲线斜率会变化，难以多年预测
- 该值由 status_line 一行单独展示（`展期收益 +X%`）
- 期货合约表的基差列也提供直观参考
