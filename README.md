...
> **估值低（PE/PB 历史分位都 <50% 且 PBS 低于趋势线）且曲线健康（roll_yield > 0，backwardation）时买；估值过高（>85%）或曲线异常（roll_yield ≤ 0）时卖。**
...
| **PBS** | 3143 | close/pb ≈ 隐含净资产 B，剥离估值周期只留净资产增长 |
| **PBS_score** | 0.99 | PBS_fair/now，>1 资产折价(便宜)，<1 资产溢价(贵) |
| **基差** | -49.2 点 | 期货收盘 - 现货收盘；**负值=贴水**（期货便宜）|
| **年化贴水** | +7.2% | 把基差按剩余天数年化，便于跨合约比较 |
| **展期收益** | +1.2% | roll_yield = 下月年化贴水 − 当月年化贴水（曲线斜率），>0 才能展期吃价差 |
| **Carry Score** | 60 | 滚贴水持有评分(满分100)=下季贴水(40)+PB分位(25)+PBS_score(25) |
...
```
csi1000-monitor/
├── monitor.py        ← CLI + 流水编排 + 多区间分位 + PBS 底部回归 + Carry Score 计算
...
[2] _generate_report() — 读 DB → 评估信号 → 写 signals 表 + 渲染报告
    ...
    │   ├─ _compute_bottom_trend()     — PBS 底部回归（隐含净资产对数趋势线 + 理论点位 + PB 分位情景）
    │   ├─ _compute_carry_score()      — IM Carry Score（下季贴水+PB分位+PBS_score）
    │   ├─ _compute_expected_return() — 三因子预期收益（贴水+ROE+分红+估值变动）
    │   ├─ fetch_otm_call()           — 实时拉期权算 BS 增厚
    │   └─ → {pe_ttm, pe_ttm_pct, pb_pct, bottom_trend, carry_score, expected_return, contracts, otm_call, ...}
    │
    ├─ _extract_signal_metrics()
    │   抽出 signals 需要的 7 个数：pe_ttm_pct_10y / pb_pct_10y / pbs_score /
    │   current_month_discount / current_month_days / next_month_discount / roll_yield
    │   （roll_yield 是策略判断主指标，pbs_score/pb_pct_10y 是 entry 新增条件）
    │
...
| 持仓 | 信号 | priority | 触发条件 |
|---|---|---|---|
| 空仓 | `entry` | 2 | 四条件全满足：PE<50% **且** PB<50% **且** PBS_score>1.05 **且** roll_yield>0 |
| 空仓 | `warn_entry` | 4 | 部分条件满足但未全达标（列出 PE/PB/PBS/roll 四条件 ✓/✗ 缺口）|
| 空仓 | `wait` | 5 | 兜底（其他情况）|
...
**entry 四条件的逻辑**：把"资产底"和"价格底"两轴都纳入判断——PE/PB 分位管价格端（PE 看盈利、PB 看资产），PBS_score 管资产端（资产是否低于长期趋势线），roll_yield 管展期收益。四条件全满足才入场，任一不达标 fall through 到 warn_entry/wait。
...
报告里的"操作建议"列出**所有 priority 最小的 suggestion**。比如 `reduce_pe` 和 `reduce_basis` 同 priority=1 双触发时，两条建议都展示，不会丢一条。此外附一行 **IM Carry Score 档位建议**（区分持仓/空仓，见 §8.6）。
...
| PE 静态 | 同上 | "静态市盈率"（加权）→ `pe_static`（仍入库，但不再算分位展示）|
...
**PBS = close / pb ≈ 指数隐含净资产 B**。剥离了 PE/PB 周期波动，只保留净资产复利增长。

对 2014-10（PB 数据起点）至今的 PBS **局部低点**（±20 日窗口内最低，约 31 个点）做**对数回归** `ln(PBS) = a + b×t`，拟合"净资产底部抬升趋势线"。对数回归契合净资产指数增长（R²≈0.90，远优于纯点位回归的 0.46）。

**为什么用 PBS 不用纯点位**：纯点位掺杂估值周期波动，规律性差；PBS 剥离周期后底部抬升规律清晰。

报告展示：
- **底部抬升率 b**：趋势线年化斜率（净资产底部每年抬升 %）
- **PBS_score = PBS_fair / PBS_now**：>1 资产折价(便宜)，=1 正常，<1 资产溢价(贵)。entry 信号用它判"资产是否低于趋势线"
- **回归框架理论点位表**：PBS 偏离比（历史 PBS_now/PBS_fair）的 50%/25%/10% 分位 × 当前趋势线 × 当前 PB = 各分位对应点位

**与 PB 分位情景点位对称**：那块固定资产 B 看 PB 分位（估值轴），这块固定 PB 看 PBS 偏离分位（资产轴）。两表合看 = 完整的"资产底 × 价格底"双轴估值。
...
|---|---|---|---|
| 2.31 | 7260 | — | 当前 |
| 2.60 | 8171 | +13% | PB 50%分位 |
| 1.93 | 6066 | -16% | PB 10%分位 |

用历史 PB 分位（而非固定 1.5/1.8）定义情景，跨周期可比。回答："资产已便宜，但估值还有多少杀跌空间"。

### 7. IM Carry Score（滚贴水持有评分）

满分 100 的三因子评分，量化"收益（贴水）+ 安全（PB/PBS）"。滚贴水≠价值投资，最怕高估值+低贴水+波动上升，Carry Score 综合区分：

| 因子 | 满分 | 档位 | 分值 |
|---|---|---|---|
| **下季贴水年化** | 40 | ≥10% / 5~10% / <5% | 40 / 30 / 10 |
| **PB 10y 分位** | 25 | <30% / 30~60% / ≥60% | 25 / 15 / 5 |
| **PBS_score** | 25 | >1.05 / 0.85~1.05 / <0.85 | 25 / 15 / 0 |
...
**贴水用下季合约**（如 IM2612，较长期限稳定），不是 roll_yield（曲线斜率，更适合信号判断）。Carry Score 不替代信号系统，作为"持有视角"补充——信号看入场/减仓时机，Carry 看当前值不值得持有。
...
carry_pbs_low_trend: float = 1.05    # PBS_score >此值 → 25 分（也作 entry 的 PBS 门槛）
carry_pbs_high_trend: float = 0.85   # 0.85~1.05 → 15 分；<此值 → 0 分
...
BOTTOM_WINDOW_DAYS = 20              # PBS 局部低点检测窗口（±交易日）
```
