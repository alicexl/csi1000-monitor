...
6. **资金测算（1 手 IM）**：合约名义价值（点位×200 元/点）、开仓保证金（15% 估算，**开仓即风险度 100% 无下跌缓冲**）、追保触发（**价格低于基准价即风险度 >100%**）、权益归零线（不加钱跌 15% 权益归零，再跌穿仓）、各 PB 情景两列：浮亏（每跌 1 点 200 元/手）+ **补足至 100% 风险度需追加 = 浮亏×85%**（追加后权益 = 新保证金占用）、建议总资金 = 保证金 + 扛 -1σ 浮亏（备足后 -1σ 风险度 = p_1σ/基准价 <100%，无需追加）；基准价空仓=现价（假设现价开仓）、持仓=入场价；多手持仓按倍数放大
7. **贴水覆盖性**：持有 1 年的展期贴水（按当前下季贴水线性累计）vs PB -1σ/-2σ 跌幅，逐格标 ✅已覆盖/❌未覆盖（贴水只看 1 年：-1σ 主判常态杀跌应覆盖，-2σ 极端参考不要求；3 年才覆盖说明下行保护不足）
8. **期货合约**：IM 当月/下月/当季/下季的基差和年化贴水率
9. **卖 Call 增厚**（持仓专属）：1σ OTM call 的执行价/≈σ/ATM IV、权利金、年化增厚率、行权概率
...
阈值在 `signals.py` 的 `Thresholds` dataclass（入场/平仓阈值）+ `monitor.py` 顶部的 `PCT_WINDOWS` / `FIT_MIN_POINTS`（BPS 回归最少点数）。资金测算口径在 `monitor.py` 顶部 `IM_MULTIPLIER`（200 元/点）/ `IM_MARGIN_RATE`（15% 估算，期货公司可能上浮）。
