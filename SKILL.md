...
5. **PB 分位情景点位 + 资金测算（1 手 IM）**：固定 B 看 PB 经验分位(50%/15.9%(-1σ)/1%(-2σ)≈2024 危机支撑区 1.60)情景对应点位与跌幅（估值杀跌风险，σ 概率标签标"出现难易"）；同表附资金列——合约名义价值（点位×200 元/点）、开仓保证金（15% 估算，**开仓即风险度 100% 无下跌缓冲**）、各情景浮亏（每跌 1 点 200 元/手）+ **补足至 100% 风险度需追加 = 浮亏×85%**（追加后权益 = 新保证金占用）、建议总资金 = 保证金 + 扛 -1σ 浮亏（备足后 -1σ 风险度 = p_1σ/基准价 <100%，无需追加）；基准价空仓=现价（假设现价开仓）、持仓=入场价；多手持仓按倍数放大
