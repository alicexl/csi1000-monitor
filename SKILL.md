...
6. **资金测算（1 手 IM）**：合约名义价值（点位×200 元/点）、开仓保证金（15% 估算）、保证金耗尽线（不加钱跌 15% 亏光）、各 PB 情景需补缴资金（每跌 1 点浮亏 200 元/手，按浮亏全额备足）、建议总资金 = 保证金 + 扛 -1σ 补缴；基准价空仓=现价（假设现价开仓）、持仓=入场价；多手持仓按倍数放大。实际追加保证金按结算价计 ≈ 浮亏×85%（保证金占用随价格同步下降 15%）
